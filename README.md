...
**자동 분류 동작 방식** — 카테고리를 비워두고 기록해도 앱이 스스로 분류합니다: ① 같은 장소를 과거에 라벨한 이력이 있으면 그 카테고리를 상속 ② 없으면 키워드 규칙(병원→의료비, KTX→여행 등) ③ 그래도 모르면 '기타'. 추론된 건은 데이터에 `inf:1` 표시가 남습니다. 규칙 수정: `tools/parse.py`와 `Code.gs`의 `KEYWORD_RULES`.
